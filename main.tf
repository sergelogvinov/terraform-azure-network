locals {
  ...
  network_subnet_v4 = { for idx, zone in var.regions : zone => cidrsubnet(local.network_cidr_v4, 2, idx) }
  network_subnet_v6 = { for idx, zone in var.regions : zone => cidrsubnet(local.network_cidr_v6, 2, idx) }
}
...
resource "azurerm_subnet" "controlplane" {
  ...
  address_prefixes = [
    for cidr in azurerm_virtual_network.main[each.key].address_space : cidrsubnet(cidr, length(split(".", cidr)) > 1 ? 4 : 6, 0)
  ]
...
resource "azurerm_subnet" "shared" {
  ...
  virtual_network_name = azurerm_virtual_network.main[each.key].name
  address_prefixes = [
    for cidr in azurerm_virtual_network.main[each.key].address_space : cidrsubnet(cidr, length(split(".", cidr)) > 1 ? 4 : 6, 1)
  ]
  private_endpoint_network_policies = "Enabled"
}
...
resource "azurerm_subnet" "public" {
  ...
  virtual_network_name = azurerm_virtual_network.main[each.key].name
  address_prefixes = [
    for cidr in azurerm_virtual_network.main[each.key].address_space : cidrsubnet(cidr, length(split(".", cidr)) > 1 ? 2 : 6, 2)
  ]
  service_endpoints                 = ["Microsoft.ContainerRegistry", "Microsoft.Storage.Global", "Microsoft.KeyVault", "Microsoft.Sql"]
  private_endpoint_network_policies = "Enabled"
}
...
resource "azurerm_subnet" "private" {
  ...
  virtual_network_name = azurerm_virtual_network.main[each.key].name
  address_prefixes = [
    for cidr in azurerm_virtual_network.main[each.key].address_space : cidrsubnet(cidr, length(split(".", cidr)) > 1 ? 2 : 6, 3)
  ]
  service_endpoints                 = ["Microsoft.ContainerRegistry", "Microsoft.Storage.Global", "Microsoft.KeyVault", "Microsoft.Sql"]
  private_endpoint_network_policies = "Enabled"
}
...
resource "azurerm_route_table" "main" {
  ...
  dynamic "route" {
    for_each = [for cidr in azurerm_virtual_network.main[each.key].address_space : cidr]

    content {
      name           = "${var.network_name}-${each.key}-v${length(split(".", route.value)) > 1 ? "4" : "6"}"
